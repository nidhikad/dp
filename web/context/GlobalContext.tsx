...
import { normalizeKbName } from "@/lib/knowledgeBase";
import {
  initializeTheme,
  setTheme,
  getStoredTheme,
...
export function GlobalProvider({ children }: { children: React.ReactNode }) {
  ...
  const startSolver = (question: string, kb: string) => {
    ...
    const kbNorm = normalizeKbName(kb);
  ...
  const sendChatMessage = (message: string) => {
    ...
    const enableRag = chatState.enableRag;
    const kbForWs = enableRag
      ? normalizeKbName(chatState.selectedKb)
      : chatState.selectedKb.trim() || "";
