...
export default function HomePage() {
  ...
  useEffect(() => {
    fetch(apiUrl("/api/v1/knowledge/list"))
      .then((res) => res.json())
      .then((data) => {
        const items = parseKnowledgeListResponse(data);
        const names = knowledgeNamesOrFallback(items);
        const kbList: KnowledgeBase[] = names.map((name) => {
          const row = items.find((i) => i.name === name);
          return { name, is_default: row?.is_default };
        });
        setKbs(
          kbList.length > 0
            ? kbList
            : [{ name: DEFAULT_KB_NAME, is_default: true }],
        );
        if (!chatState.selectedKb) {
          const defaultKb = items.find((kb) => kb.is_default)?.name ?? names[0];
          setChatState((prev) => ({
            ...prev,
            selectedKb: defaultKb ?? DEFAULT_KB_NAME,
          }));
        }
      })
