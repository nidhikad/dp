...
export default function KnowledgePage() {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(apiUrl(`/api/v1/knowledge/${encodeURIComponent(targetKb)}/upload`), {
        method: "POST",
        body: formData,
      });
      if (!res.ok) {
        const msg = await parseApiError(res);
        showToast(msg || t("Upload failed"), "error");
        return;
      }
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        const msg = await parseApiError(res);
        showToast(msg || "Creation failed", "error");
        setUploading(false);
        return;
      }
